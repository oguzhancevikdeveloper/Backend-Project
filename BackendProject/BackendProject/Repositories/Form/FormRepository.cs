...
{
    public async Task<List<Models.Form>> GetFormsAsync(string searchTerm = "")
    {
    ...
    public async Task<Models.Form> GetFormByIdAsync(int id)
    {
        return await _context.Forms.Include(f => f.Fields).FirstOrDefaultAsync(f => f.Id == id);
    }

    public async Task AddFormAsync(Models.Form form)
    {
